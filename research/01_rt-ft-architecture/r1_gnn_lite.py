#!/usr/bin/env python3
"""
R1 GNN Lite: Lightweight GNN decoder for practical runtime
============================================================

Key changes from r1_gnn_fast.py:
  - Smaller model: hidden=32, n_layers=3 (5K params vs 20K)
  - Less training data: 10K (d=3), 5K (d=5)
  - Epochs: 40 for both d=3 and d=5
  - MPS memory management: cache clearing per epoch to prevent OOM on 16GB Mac
  - Focus on the key question: does GNN beat MWPM at d=5 with rho≥0.05?

Estimated runtime: ~45min total
seed=42
"""
import gc
import numpy as np
...
class GNNLite(nn.Module):
    """Lightweight GNN: 3 layers, hidden=32. ~5K params."""
...
def _clear_mps_cache():
    """Clear MPS/GPU cache to prevent OOM on 16GB Mac."""
    if DEVICE == 'mps':
...
def main():
    ...
    print("="*70)
    print("  R1 GNN LITE: Lightweight batched GNN decoder")
    print(f"  Device: {DEVICE}")
    print("="*70)

    V = 0.1417  # Phase 1 (sigma_eff=8.5dB)
    ...
    for d in [3, 5]:
        ...
        for rho in rho_list:
            ...
            # Reduced data sizes for practical runtime
            ...
            print(f"    MWPM: {mwpm_pL:.4e} ({mwpm_e.sum()}/{n_te}) [{dt_m:.1f}s]")
            ...
            with open(os.path.join(OUT_DIR, 'r1_gnn_lite_results.json'), 'w') as f:
    ...
    # Summary table
    print(f"\n  {'d':>2} {'rho':>5} {'MWPM':>10} {'GNN':>10} {'Ratio':>7} {'Win':>5}")
    print("  "+"-"*50)
    ...
    # Key comparison: d=3 vs d=5 scaling
    print("\n  KEY: Does GNN advantage scale with d?")
    for rho in [0.05, 0.08, 0.10, 0.15]:
        r3 = results.get(f'd3_rho{rho:.2f}', {}).get('ratio', 0)
        r5 = results.get(f'd5_rho{rho:.2f}', {}).get('ratio', 0)
        if r3 > 0 and r5 > 0:
            scale = "GROWS" if r5>r3 else "shrinks"
            print(f"    rho={rho:.2f}: d=3 {r3:.2f}x, d=5 {r5:.2f}x → {scale}")
