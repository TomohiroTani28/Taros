...
**含意：** V_eff推定の高精度（§III-B）はデコーダ改善に直結するのではなく、(1) 異常検出（§III-C）、(2) 再校正トリガー、(3) チャネル非対称検出、(4) ハードウェアヘルスモニタリングに活用すべきである。デコーダ性能改善にはGNN等のスケール不変性に制約されないアーキテクチャが必要である。
...
[5] (Nature Communications 2026), "Real-time monitoring of multimode squeezing."
