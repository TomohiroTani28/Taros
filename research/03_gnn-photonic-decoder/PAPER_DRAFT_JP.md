...
ρとハードウェアパラメータの対応は、ポンプRINのパワースペクトル密度 S_RIN [dB/Hz] から以下で導出される[8]。TDMクロック f_clk = 100 MHz、WDMチャネルアイソレーション I_wdm [dB] として、

ρ ≈ 10^(S_RIN/10) × f_clk × 10^(−I_wdm/10)
...
Stim 1.15.0[2]によるrotated surface code検出器エラーモデル（DEM）生成、PyMatching 2.3.1[3]によるMWPMデコーディング。GKP変位ノイズと相関ノイズをカスタム実装し、DEMのエッジ構造上でGKP残差とシンドロームを生成した。全実験seed=42で再現可能。コードは[URL]で公開。
...
| 0.03 | 89 / 73 | 1.22× [0.89, 1.67] | 22 / 5 | **4.40×** [1.67, 11.6] |
| 0.05 | 110 / 70 | 1.57× [1.16, 2.13] | 24 / 7 | **3.43×** [1.47, 7.99] |
| 0.08 | 128 / 55 | **2.33×** [1.69, 3.20] | 36 / 7 | **5.14×** [2.27, 11.7] |
| 0.10 | 135 / 38 | **3.55×** [2.47, 5.12] | 64 / 3 | **21.3×** [6.73, 67.5] |
| 0.15 | 224 / 35 | **6.40×** [4.47, 9.16] | 167 / 4 | **41.8×** [12.0, 145] |

d=7（n_test=10,000）: ρ=0.03〜0.10で残差減算エラー0（p_L ≤ 3.0×10⁻⁴、95%ポアソン上界）。ρ=0.15でvanilla 38 → 残差減算 2（19.0×、95% CI: [4.5, 80.1]）。
...
| 0.15 | 224 / 50 | **4.48×** [3.27, 6.13] | 167 / 5 | **33.4×** [13.7, 81.6] |

d=7（n_test=10,000）: ρ=0.10で19 / 1（19.0×、95% CI: [2.6, 145]）。ρ=0.03〜0.08およびρ=0.15でGNNエラー0（p_L ≤ 3.0×10⁻⁴、95%ポアソン上界）。
...
| 0.08 | 141 / 101 | **1.40×** | 35 / 8 | **4.38×** | 9 / 0 | **≥30×** |
| 0.10 | 139 / 86 | **1.62×** | 62 / 7 | **8.86×** | 23 / 0 | **≥77×** |
...
2. **残差減算MWPMが共通モード型相関に最強。** d=5で最大41.8倍（95% CI: [12.0, 145]）、d=7で19.0倍。実装コストゼロ。ただしρ=0では悪化（d=3で0.84倍）し、共通モードモデルを仮定する。

3. **GNNが距離とともにスケール。** d=5で33.4倍（95% CI: [13.7, 81.6]）。4,033パラメータの軽量さ（AlphaQubit比2,500分の1）で相関構造をモデルフリーに学習。ρ=0ではvanillaに劣位（d=3: 0.60×、d=5: 0.75×）——相関デコーダの限界を正直に報告する。
...
[10] Google Quantum AI, "Learning to decode quantum error correction with a recurrent transformer," Nature (2024). [AlphaQubit]

[11] V. Sivak et al., "Real-time quantum error correction beyond break-even," Nature **616**, 50 (2023).

[12] P. Bausch and F. Leditzky, "Error-corrected fermionic quantum processors with neutral atoms," arXiv:2501.17637 (2025).
