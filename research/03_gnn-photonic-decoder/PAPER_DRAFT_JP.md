...
先行研究[1]はCV系の正しいノイズモデル（現象論的、soft-info MWPM）を同定したが、相関ノイズ下でのデコーダ最適化は未解決であった。ニューラルデコーダ[5,10,11,13]およびコード設計の進展[12]がDV系で有望な結果を示す一方、CVフォトニック系の相関ノイズに対する体系的評価は行われていない。
...
1. **No-go result: V_eff適応の原理的不可能性**: V_eff再校正MWPMは170,000ショットで全ショットvanillaと判定一致。一様なノイズスケーリングに対するMWPM適応は原理的に無効であり、この盲点を埋める
...
本研究ではPhase 1動作点を使用する: σ_eff = 8.5 dB（σ_gen = 13 dB、L = 0.39 dB、V_nl = 0.010 SNU 込み）、V_eff = 0.1417 SNU、p_phys = 9.28×10⁻³（付録B参照）。
...
| d | DEM edges (全体/使用†) | 検出器数 | n_train | n_test | rounds | epochs |
|---|------------------------|---------|---------|--------|--------|--------|
| 3 | 76 / 66 | 24 | 5,000 | 30,000 | 3 | 40 |
| 5 | 418 / 334 | 120 | 5,000 | 30,000 | 5 | 40 |
| 7 | 1,224 / 954 | 336 | 3,000 | 10,000 | 7 | 40 |

†Stim DEMの全エラーエントリ数 / `extract_graph()` が1-detector・2-detectorエラーのみを抽出した実使用エッジ数。本研究の全シミュレーションは後者（66/334/954）を使用。
...
各デコーダのp_Lに対する95%信頼区間はWilson score interval で計算した。比（改善率）の95%信頼区間は、分子・分母のWilson区間端点から保守的に構成した（ratio CI = [p₁_lo/p₂_hi, p₁_hi/p₂_lo]）。エラー数0の条件ではポアソン分布の95%上界 3/n_test（rule of three）を用い、改善下界を「≥X×」と報告した。低カウント（k ≤ 5）の比CIは幅が広く（例: 4/30,000 vs 167/30,000 で CI = [14.0, 125]）、点推定値は参考として精密な比率推定にはより大きなn_testが必要である。
...
**(ii) 高dほど改善が大きい。** d=3: 6.4倍、d=5: 41.8倍（@ρ=0.15）。実使用DEMエッジ数の増加（d=3: 66、d=5: 334、d=7: 954）に伴い、共通モードの推定精度が向上するためである（標準誤差 ∝ 1/√N_edges）。
...
**d=5では共通モード型相関の全ρ>0で残差減算がGNNを上回る。** 334エッジ（実使用数）の平均による共通モード推定が極めて精確であり、かつ実装コストがゼロ（ソフトウェア変更のみ、FPGA追加リソース: 加算器+除算器）であるため、**「同じ性能水準ならより単純な手法が勝つ」という工学的原則に従い、共通モード型相関ではDecoder 3を推奨する。**
...
| 0.15 | 242 / 103 | **2.35×** | 159 / 13 | **12.2×** | 45 / 1 | **45.0×** [5.9, 341] |
| **0.20 (OOD)** | 337 / 109 | **3.09×** | 248 / 25 | **9.92×** | 83 / 4 | **20.8×** [6.5, 66.1] |

（d=7 GNNエラー0の条件: 改善下界を95%ポアソン上界 3/n_test から計算し「≥X×」と表記）

**OOD ρ=0.20 での残差減算との比較**（追加実験）:

| d | Vanilla (err) | 残差減算 OOD | Mixed-ρ GNN OOD | 残差減算改善 | GNN改善 |
|---|------|------|------|------|------|
| 3 | 310/30K | **38** | 109 | **8.2×** | 3.1× |
| 5 | 276/30K | **12** | 25 | **23.0×** | 9.9× |
| 7 | 99/10K | **4** | 4 | **24.8×** | 20.8× |

**残差減算はOOD ρ=0.20でもGNNを上回る（d=3: 8.2× vs 3.1×、d=5: 23.0× vs 9.9×）。d=7では同等（24.8× vs 20.8×）。** 共通モード型相関であれば、ρが訓練分布外であっても残差減算の原理（残差平均による共通モード推定）は物理的に有効であり、ML学習に依存しない。

ただしこの結果は、相関構造が共通モードモデルに従う場合に限定される。実運用で相関構造が変化する環境（例: WDMチャネル劣化による空間的不均一相関）ではMixed-ρ GNNが優位となる可能性がある。
...
- **残差減算**: 実使用DEMエッジ数が66→334→954と増加。共通モード推定量 r̄ = (1/N)Σr_j の標準誤差は σ/√N に比例して減少。d=7で954エッジの平均は精確な共通モード推定を提供する
...
3. **GNN推論レイテンシ**: ~6ms/shot（d=3, Apple MPS）はリアルタイムQEC（~10ns/cycle）に対して10⁶倍遅い。FPGA/ASIC実装によるリアルタイムデコーディング[16,17]が実運用には必須。AlphaQubit[10]はTPU依存であり同様の課題を抱える
...
1. **No-go result: V_eff再校正は原理的に無効。** 170,000ショットで検証。MWPMのスケール不変性により、一様なV_eff適応はデコード結果を変えない。この結果は「ノイズパラメータを正確に知ればデコーダが改善する」という直感的期待を否定し、相関ノイズ下でMWPMを改善するにはスケール不変性を迂回する手法（残差減算またはML）が必要であることを示す。
...
4. **残差減算はOODでもGNNを上回る。** OOD ρ=0.20でd=5: 残差減算23.0× vs GNN 9.9×、d=7: 24.8× vs 20.8×。共通モード型相関では、ρが訓練分布外であっても残差減算の原理が物理的に有効であり、ML学習に依存しない。GNNの真の価値は、相関構造が非共通モード型の場合にモデルフリーで適応できる点にある。

残差減算とGNNは相補的である。共通モード型相関（ρ>0）ではOODを含め残差減算が最強。相関構造が未知または非共通モード型のときGNNが優位。実運用ではρ推定器（CUSUM等）との組み合わせによるデコーダ自動切り替え——ρ=0ではvanilla、ρ>0で共通モード型なら残差減算、構造不明ならGNN——が最適戦略である。
...
- 設計文書（06_noise-budget.md）の全損失パス通過後のBSモデル計算
- η = 0.914、V_sqz = 0.0501、V_nl = 0.010 SNU
- V_eff = 0.914 × 0.0501 + 0.086 + 0.010 = 0.142 ≈ 0.1417 SNU
- σ = √(0.1417/2) = 0.2662
- p_phys = erfc(√π/(4 × 0.2662))/2 = 9.28 × 10⁻³

本研究の全シミュレーションは V_eff = 0.1417 を使用。
...
**DEMエッジ数について**: Stim DEMの全エラーエントリ数（d=3: 76、d=5: 418、d=7: 1,224）と、`extract_graph()` が1-detector/2-detectorエラーのみ抽出した実使用エッジ数（d=3: 66、d=5: 334、d=7: 954）は異なる。差分は0-detector（境界のみ）や3-detector以上のエラーエントリである。本研究の全シミュレーションは実使用エッジ数（66/334/954）で実行されており、全エラーカウントは同一の `run_unified.py` スクリプト（seed=42）から生成され内部的に整合している。
