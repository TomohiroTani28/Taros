...
### 2.1 レイテンシチェーン（v1.3: Flash ADC + Pipeline ADC 並列構成）
...
| (4) DAC | **MAX5898 (14bit 400MSPS, パラレルIF)** | **4ns** | パラレルバスIF、JESD不使用 [v3.2: AD9117(250MSPS上限)→MAX5898に変更。group delay 2cycle@400MHz] |
| (5) EOドライバ + EOM応答 | iXblue MPZ-LN-10 + LNOI MZM | 3ns | 帯域12GHz, EO効果~fs応答 |
| **合計(設計ベースライン 400MHz)** | | **25.5ns ≈ 26ns** | **3モード遅延 (100MHz時)** [v3.2: DAC 5→4nsで-1ns] |
| **合計(楽観ケース 500MHz)** | | **22ns** | **2-3モード遅延 (100MHz時)** |

**注 (v2.2)**: S&H回路(AD783, 2ns acquire)はPLL誤差信号サンプリング用(05_phase-lock.md §5.6)であり、FF-1高速パスには含まれない。FF-1はTIA→Flash ADC→FPGA→DAC→EOMの直結パスで動作し、S&Hを経由しない。
...
■ 楽観ケース: 500MHz (Versal PL domain, 2ns/cycle)
  ...
  FF-1合計(楽観ケース 500MHz): TIA 3ns + ADC 3ns + FPGA 8ns + DAC 5ns + EOM 3ns = **22ns → 2-3モード遅延** (v2.2: 旧21nsは内訳計算と不一致だったため22nsに修正)
  ※ADCは500MHzケースでも3ns維持（デバイス物理遅延は変わらない）。旧値2nsはデバイス仕様外。

  **500MHz timing closure検証計画 (v1.5追加)**:
...
**v1.3変更点**: soft info計算を高速パスから分離。hard decisionのみ設計ベースライン(400MHz)5 cycle / 楽観ケース(500MHz)4 cycleで完了。

### 2.4 レイテンシ vs TDMクロック (v1.4: Flash ADC構成, 設計ベースライン400MHz)
...
**モードスロット内割り当て (v3.2追加)**: 10ns TDMスロット = パルスOPA照射 3ns (t=0-3ns) + ホモダイン測定窓 7ns (t=3-10ns)。
Flash ADC (1.5GSPS) は7ns窓内に10-11サンプル取得。TIA整定時間(0.7ns)後の有効測定窓は~6ns。

**タイミング注記 (v1.4)**: ADCサンプル完了がモードスロット末尾(t≈9ns)の場合、
...
[Stage 1: GKP内部デコーダ] ← 18.5ns (TIA 3ns + ADC 3ns + FPGA 12.5ns, FF-1内に含む, 設計ベースライン400MHz)
    ...
    │
    ├── bit値 → FF-1 (基底選択) : 27ns total (v1.4: 設計ベースライン400MHz, Flash ADC + パラレルDAC)
    │
...
| 方式 | レイテンシ | 根拠 | TRL |
|------|----------|------|-----|
| 電気的FF (Phase 0-1) | **27ns** (設計ベースライン) / 22ns (楽観) | 本設計 v1.4 | 6 |
...
GKP:   ▒    ▒    ▒    ▒   ...  ▒   ...  ▒    ← GKPデコード(Stage1)
       │         │    │        │        │
FF-1:  ├──27ns──────┤  │        │        │    ← 基底角度設定(v1.4, 設計ベースライン400MHz, mode k+3に適用)
       │              │        │        │
       └──────────────┴────────┴────────┘
...
*本文書はCV方式のフィードフォワード制御チェーンの全レイテンシバジェットを定義し、100MHz TDMクロックでの動作可能性を実証したものである。v1.4: TI ADC06D1500(6bit低レイテンシADC (folding/interpolating型))+Pipeline ADC(14bit)並列構成+パラレルDAC(AD9117)により設計ベースライン(400MHz)27nsレイテンシ（3モード遅延）、楽観ケース(500MHz)22ns（2-3モード遅延）を達成。macronode latticeの構造上許容可能。*
