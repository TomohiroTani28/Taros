...
| Phase 2+ (理論限界) | 13dB | 0.15dB | 0.966 | 0.082 | 10.8dB | 全条件達成時（QE99%, 結合0.02dB/facet） |
...
**出典**: arXiv:2603.02744 (2025), LIGO技術転用

PPLN導波路OPAの主要損失源は**空間モードミスマッチ**（全損失8%中4%）であることがLIGO関連研究で特定された。機械学習制御SLM（空間光変調器）で導波路出力モードをホモダイン検出器のLOモードに精密整合させることで、損失を1%以下に低減可能。

| パラメータ | 現行 | ML-SLM適用後 |
|-----------|------|-------------|
| 空間モードミスマッチ損失 | ~4% (0.18dB) | <1% (0.04dB) |
| σ_gen | 12.1dB (実測) → 13dB (目標) | 同左 |
| **σ_eff (Phase 1 離散, L=0.39dB)** | **≈8.5dB** | **≈9.1-9.5dB** |
| σ_eff (Phase 2+ PIC, L=0.27dB) | ≈9.3dB | ≈9.5-9.7dB (PICで既にモード整合最適化済みのため改善幅小) |
| 追加コスト | — | SLM ~30万円 + カメラ ~10万円 |
| 実装難易度 | — | 低（OPA出力とLO間にSLM挿入） |

**Phase -1 T0a（OPA単体評価）に追加推奨**。離散光学系（Phase -1/0/1）でのσ_eff +0.6-1.0dB改善はbreak-evenマージンを拡大し、Go/No-Go判定に影響する。Phase 2+ PICでは既にモード整合が最適化されるため効果は限定的。

### 6.2 非エルミート例外点増強（Phase 1+研究候補）

**出典**: arXiv:2507.17961 (2025)

例外点（EP）近傍ではパラメトリック増幅感度が通常の2乗→**4乗スケーリング**に向上する。OPAポンプパワーをEP近傍に精密制御することで、σ_gen = 12dB → **実効15dB+** が理論的に到達可能。Phase 1以降の研究対象として監視。

### 6.3 GKP低スクイージング入力ブリーディング

**出典**: arXiv:2409.06902 (2024)

入力スクイージング**3dB未満**から反復ブリーディングで**9.75dB等価GKP状態**を生成する手法。TarosのOPA（12.1dB+）と組み合わせれば、GKP忠実度の大幅向上が可能。Step C（QD deterministic breeding）との統合が有望。
