...
PPLN導波路OPA + macronode TDMの「CV pure」構成は、冷却不要・低消費電力（104-112W, WDM込み）であり、**ラックすら不要**。表面符号距離dの選択（=PMF遅延線の長さ）のみで性能が決まるため、3段階のプロダクトラインを単一プラットフォームから展開可能。

**設計目標**: ACアダプタ駆動（USB-PD EPR 28V/5A=140W必須、消費104-112W）の誤り訂正型量子コンピュータ
...
リソースの80%をCVに集中。Phase -1のCV判定（G-EXP1: 装置BOM約560万円/約2ヶ月、人件費込み完全コスト約2,900万円）で成否判定後、DV fallback経路を維持。
...
|--------|---------|---|---------|
| 表面符号距離 d | 3 | 5 | 7 |
| 論理エラー率 p_L (MWPM, Phase2+ PIC) | ~10⁻² | ~10⁻³ | **3.3×10⁻⁴** (L=0.27dB) |
...
| 論理ゲートレート (単一ch, strict) | ~1.9kHz | ~400Hz | ~120Hz |
| 論理ゲートレート (WDM並列, strict)†† | ~10kHz (5ch) | ~3kHz (8ch) | ~840Hz (7ch)† |
...
P_round≈10⁻³は「GKP格子点に高信頼度で着地したモードの割合」であり、低品質モードの
physical error rateが50%を超えるわけではない（全モードでp_phys≈4.9×10⁻³, Phase 2+ PIC現実的 L=0.27dB）。

- **高品質モード (0.1%)**: GKP格子中心付近、信頼度高、表面符号で通常qubitとして使用
- **低品質モード (99.9%)**: GKP格子付近だが不確定性大、低重みで使用

全モードのp_phys≈4.9×10⁻³(Phase 2+ PIC, L=0.27dB, BSモデル)であり、「erasure率99.9%」ではない。表面符号の入力は
