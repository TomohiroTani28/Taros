...
| # | 理由 | 定量的根拠 |
|---|------|-----------|
| 1 | 冷凍機排除 = コスト100倍削減 | 超伝導$3-5M必須 → CV室温動作 |
...
| 5 | 競合不在の市場 | $120K帯FTQC製品は世界に存在しない |

リソースの80%をCVに集中。Phase -1のCV判定($80K/4ヶ月)で成否判定後、DV fallback経路を維持。
...
|--------|:---:|:---:|:---:|
| 表面符号距離 d | 3 | 5 | 7 |
| 論理エラー率 p_L (MWPM, Phase2+ PIC) | ~10⁻² | ~10⁻³ | **3.3×10⁻⁴** (L=0.27dB) | [v3.6: erfc+V_nl修正。理論限界6.1×10⁻⁷(L=0.15dB)]
...
| 消費電力 (WDM込み) | **~104W** | **~109W** | **~112W** | [v3.5: Edu OPA×2必須(macronode要件)]
| 冷却 | パッシブ+低速ファン(<25dBA) | パッシブ+低速ファン(<25dBA) | パッシブ+低速ファン(<20dBA) |
| 原価 (量産100台, WDM込み)‡ | **$56K** | **$75K** | **$95K** |
| 販売価格 | **$90K** | **$120K** | **$170K** |
| 用途 | 教育、QEC研究 | 研究、VQE/QAOA | FTQC、量子化学 |

†**Max重量内訳 (v2.2追加)**: Pro(7.5kg)からの差分+2.2kg:
...
‡原価差の主要因: Edu→Pro: ADC/PD数(10→16個, +$8K) + 筐体サイズ/加工費(+$5K) + 較正時間(+$3K)。
Pro→Max: 筐体大型化(+$5K) + PMFスプール/精密組立(+$3K) + d=7用追加較正・バーンイン(+$8K) +
ADC高精度選別(+$5K) + 長期安定性試験(+$5K)。Max原価$95Kの内訳は`12_mechanical.md`のPro BOMを
...
| カテゴリ | 内容 | コスト |
|---------|------|--------|
| OPA光源 | PPLN OPA ×2 + 1550nm GS-DFB+SOA+PPLN SHG (Option B) | $16K |
| 光学系 | PMF 200m + BS×3 + カプラ + WDM 7ch AWG | $15K |
| 検出系 | BPD ×14(7ch×2) + ADC ×14 | $25K |
| FPGA | Versal VE2302 + 基板 | $8K |
| 電源・熱 | USB-PD EPR PSU + ヒートシンク + ファン×2 | $5K |
| 筐体 | Al CNC一体 (40×30×20cm) | $6K |
| EO・制御 | EOM + PZT + PLL基板 + DAC | $8K |
| 組立・検査 | ファイバ接続 + 較正 + バーンイン | $14K |
| **合計** | | **$95K** |

*Pro BOMからの差分: 検出ch増(8→14ch, +$7K)、PMF長(102→200m, +$1K)、筐体大型化(+$2K)、高精度較正(+$6K)、AWG追加ch(+$3K)。OPA本数同一(×2)。*
...
│  v2.0 BSモデル: AWG込みL=1.42dBではσ_eff=5.0dB(閾値未満)。Phase 2+ PIC統合    │
│  (L=0.15dB)でσ_eff=10.8dB達成。旧値11.7→11.5dBはdB減算誤り(06_noise-budget参照) [v3.6]│
└─────────────────────────────────────────────────────────────────────┘
```
...
| 追加コスト | $9K (RF driver $3K + EOM ×2 $4K + EDFA $2K) |
...
| 項目 | 消費電力 |
|------|---------|
| Master LD (1550nm) + 1550nm GS-DFB+SOA+PPLN SHG (Option B) | ~15W (DFB 2W + SOA 8W + SHG TEC 5W) | [v3.2: SOA 5→8W修正。SOA datasheet typ.消費8W@1.6W出力]
...
| **合計 (Pro, WDM 8ch込み)** | **109W** | [v3.2: SOA 5→8W反映。旧106W→109W]

**EO comb RF電力内訳**: PM用RF ~6W + IM用RF ~8W + EDFA 3W = 17W

**モデル別電力 (v1.5更新: OPA構成反映、v1.7 RF電力修正)**:
- Taros Edu (OPA×2, WDM 5ch): 86W + 8W + 10W(RF修正) = **104W** [v3.5: OPA×2必須(macronode要件)。旧OPA×1(100W)は撤廃]
...
■ Taros Edu ベース電力導出 (OPA×2, v3.5修正):
  Pro基準 86W（OPA×2共通）= 86W [v3.5: macronode要件によりOPA×2必須。旧OPA×1(82W)撤廃]
...
**注 (v3.5修正: OPA×1→×2)**: macronode lattice生成にはBS₁で2つの独立スクイーズド真空を混合する必要がある（03_tdm-cluster.md §2.1）。**従ってEduもOPA×2が必須**。旧「OPA×1」記述はmacronode回路要件との矛盾であり撤廃。Edu OPA×2構成でもSOA 1.6W→SHG 400mW→1×2 split=200mW/OPAで閾値確保可能。Edu消費電力はOPA Peltier ×2で+4W → **104W**（USB-PD EPR 140Wで36Wマージン）。原価はOPA追加$3K → **$56K**。

**ポンプ光源の構成差異 (v2.1更新: Option B統一)**:
- **Portable (全モデル)**: 1550nm GS-DFB+SOA(1.6W peak)+PPLN SHG (Option B), SHG出力400mW peak (775nm), 消費~15W (DFB 2W + SOA 8W + SHG TEC 5W), コスト~$10K
  - OPA×2構成: SOA 1.6W peak → SHG 400mW peak → 1×2 splitter(3dB) = **200mW peak/OPA**。各OPAに閾値ポンプパワー確保。
  - ※旧仕様(SOA 800mW→SHG 200mW→splitter→100mW/OPA)ではσ_gen~9-10dBに低下し閾値未達。v2.2修正。
- **Rack (8 OPA構成)**: 同方式で高出力SOA使用、またはDFB+TA 2.5Wで対応。
- コスト構成: GS-DFB $2K + SOA $3K + PPLN SHG $3K + 制御・筐体 $2K = **~$10K**

電源: **外付けACアダプタ USB-PD EPR 140W (28V/5A) 必須**

**電源仕様**:
- **全モデル必須**: USB-PD EPR 140W (28V/5A) — 全モデルでフル性能動作、マージン31-43W
- ※旧仕様のUSB-PD 100W代替電源は廃止。Pro(109W)/Max(112W)は100W不可。Edu(100W)は100W境界のためマージン確保に140W統一を推奨。
...
材質: アルミ削り出し（放熱兼用）
仕上げ: アノダイズド（黒）
重量: 筐体3.3kg (Al A6063-T5 CNC、底板3mm+側板3mm+天板3mm+フィン18mm×43本) + 内部3.5kg + WDM 0.4kg + 振動隔離等0.2kg = 7.5kg
```
...
| 条件 | 値 |
|------|-----|
| 総発熱 | 109W (WDM 8ch込み、Pro基準) [v3.2: SOA +3W] |
...
| 内部発熱 (筐体内) | 50W (光学系+PD+RF)。残り59W(FPGA+ADC+DC-DC+EO comb RF)は天板フィン右側1/3領域HP経由外部放熱。**注(v3.2)**: OPA Peltier(8W)+SHG Peltier(4W)=12W(電気入力)のhot side排熱は電気入力+ポンプ熱≈18-20Wが筐体内に放出。上記50Wに含まれる。 | |
| マージン (ΔT=20K時) | 140W - 109W = **31W (22%)** | 十分 [v3.2: SOA +3W反映] |
...
|------|------|----------|--------|------|---------|
| IBM Quantum System Two | 超伝導 | 0 | 部屋 | $15M+ | 200kW+ |
| IonQ Forte | イオン | 0 | ラック×3 | $3M+ | 30kW |
| Xanadu Aurora | フォトニック | TBD | ラック×多 | $10M+ | 100kW+ |
| **Taros Edu** | **CV光子** | **~1 (d=3)** | **ラップトップ** | **$90K** | **100W** |
| **Taros Pro** | **CV光子** | **10+ (d=5)** | **ミニPC** | **$120K** | **109W** |
| **Taros Max** | **CV光子** | **100+ (d=7)** | **デスクトップ** | **$170K** | **112W** |
...
| Phase | 時期 | 成果物 |
|-------|------|--------|
| Phase -1 | 2026 Q2-Q4 | PPLN OPA評価 + macronode TDM PoC |
| Phase 0a | 2027 Q1-Q2 | **Taros Edu プロトタイプ** (d=3, QEC break-even) |
| Phase 0b | 2027 Q3-Q4 | **Taros Pro プロトタイプ** (d=5, FTQC入口) |
| Phase 1 | 2028 | **Taros Max プロトタイプ** (d=7, 製品級) |
| Phase 2 | 2029 | 量産設計 + 初期出荷 (100台/年) |
| Phase 3 | 2030+ | QD追加モデル (Taros Pro+, Max+) |
...
OPA予備1本 + 2×1光スイッチで劣化時自動切替。追加$3.5K。切替<1ms。

### 8.5.3 FF方式: Clifford precompile + T adaptive

- Clifford (~80%): プリコンパイル → FFレイテンシ0、モード遅延0
- T gate (~20%): 適応的FF 27ns(設計ベースライン)/22ns(楽観) → 3モード遅延 (v1.4 Flash ADC + パラレルDAC)
...
メリット: PMF 200m除去(-0.04dB損失, -$100, スプール不要で更に小型化)
...
**TDM postselectionプロトコル (v1.3明確化)**:
...
FF-1（基底選択）のみがリアルタイム27ns制約(設計ベースライン400MHz。v1.4 Flash ADC + パラレルDAC構成)。

---

*本文書はCV pure構成（QDなし、完全室温、低速ファン準無音冷却）のポータブル量子コンピュータシリーズを定義したものである。PPLN導波路OPA + macronode TDMの最小構成により、109W・7.5kg・$120Kの誤り訂正型量子コンピュータを実現する。*
