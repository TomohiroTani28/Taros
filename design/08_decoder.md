...
## 7. 相関ノイズ対応デコーダ（research/03_gnn-photonic-decoder 実証済み）

### 7.0 背景: 共通ポンプRINによる相関ノイズ

TDM方式では全モードが同一CWポンプから生成されるため、モード間に相関係数ρの変位ノイズ相関が発生する。
設計仕様ρ ≤ 0.03（DFB+TA, RIN −150 dB/Hz）。部品劣化時ρ = 0.05-0.10。

vanilla soft-info MWPMはスケール不変性により相関構造を活用できない（§7.0a証明済み）。
これに対し、2つの改善デコーダが実証された。

### 7.0a スケール不変性の数値的証明（Decoder 2: V_eff再校正MWPM）

残差分散からV_effをショットごとに推定しLLRを再計算するデコーダ。
MWPMの重みは正定数倍に対して不変であるため、V_eff再校正は**数学的にvanillaと同値**。

**数値的証明**: d=3,5,7 × 6ρ条件 × 計170,000ショットで**全ショット判定一致**（agree = 100%）。
V_eff再校正によるMWPM改善は原理的に不可能であることを確認。

### 7.0b 残差減算MWPM（Decoder 3, **実装コストゼロ、即時適用可能**）

共通モードノイズを残差の平均から推定し、減算後にLLRを再計算するデコーダ。
§7.0aのV_eff再校正（全エッジを一様にスケール → スケール不変で無効）とは**異なるアルゴリズム**:
残差そのものを修正するため、エッジごとのLLRが非一様に変化し、スケール不変性を迂回する。

> **重要**: research/03_gnn-photonic-decoder/PAPER_DRAFT_JP.md §IV-Aの「correlated MWPM max 1.04×」は
> V_eff再校正方式（Decoder 2）の結果。本セクションの残差減算方式（Decoder 3）は異なるアルゴリズムであり、
> 大幅に異なる結果を示す。論文の4者比較では両方式を区別して提示する。

**実装**: ソフトウェア変更のみ（FPGA追加リソース: 加算器1個+除算器1個）。

**統一実験結果** (Phase 1 σ_eff=8.5dB, n_test=30,000, seed=42、実験進行中のため最終JSONには全d累積予定):

| ρ | d=5 vanilla MWPM (err) | d=5 残差減算 (err) | 改善率 |
|------|----------------------|-------------------|--------|
| 0.00 | 9 | 9 | 1.00× |
| 0.03 (設計仕様) | 22 | **5** | **4.4×** |
| 0.05 | 24 | **7** | **3.4×** |
...
**メカニズム**: d=5で334 DEMエッジの平均は共通モードの高精度推定量（σ/√334 ≈ 0.055σ）。
ρ=0では共通モード成分がないため改善なし。ρ>0で共通モード除去が有効に機能。

**設計への影響**: 設計仕様ρ=0.03でもp_L(d=5)がvanilla比4.4×改善。
これは等価的にσ_effの要求を**~0.5-1dB緩和**する効果を持つ。

### 7.0c GNN Lite デコーダ（Decoder 4, Phase 2+候補）

4,033パラメータの軽量Graph Neural Network。GKP残差(r, |r|, LLR)を入力とし、
MWPMに渡す修正エッジ重みを出力。相関ノイズ構造をグラフ畳み込みで学習。

**統一実験結果** (Phase 1 σ_eff=8.5dB, per-ρ訓練, n_test=30,000):

| ρ | d=5 MWPM (err) | d=5 GNN (err) | 改善率 |
|------|---------------|--------------|--------|
| 0.00 | 9 | 12 | 0.75× (MWPMが最適) |
| 0.03 | 22 | 15 | **1.47×** |
| 0.05 | 24 | 12 | **2.0×** |
| 0.08 | 36 | 10 | **3.6×** |
| 0.10 | 64 | 4 | **16.0×** |

**Mixed-ρ単一モデル** (訓練: ρ混合, 再訓練不要でノイズ変動に適応):

| ρ | d=5 MWPM (err/30K) | d=5 MixGNN (err/30K) | 改善率 |
|------|-------------------|---------------------|--------|
| 0.08 | 35 | 8 | **4.4×** |
| 0.10 | 62 | 7 | **8.9×** |
| 0.15 | 159 | 13 | **12.2×** |
...
**GNN vs 残差減算の使い分け**:
- ρ ≤ 0.03: 残差減算で十分（GNNは独立ノイズ時にMWPMに劣る）
- ρ ≥ 0.08: GNNが残差減算を上回る可能性あり（d=3では確実にGNN優位）
- **Hardware-adaptive**: Mixed-ρ GNNは未知のρに対応可能（残差減算は共通モードモデルを仮定）
- **推論レイテンシ**: 現行~6ms/shot → FPGAまたはASIC実装で<1μsが必要（Phase 2+開発項目）

### 7.0d デコーダ選択方針（更新版）

| Phase | Primary | 相関ノイズ対応 | 根拠 |
|-------|---------|--------------|------|
| Phase -1/0 | MWPM soft-info | **残差減算MWPM** | 実装コストゼロ、ρ=0.03で4.4×改善 |
| Phase 1 | MWPM + UF hybrid | 残差減算MWPM | d=7でも954エッジの平均で高精度推定 |
| Phase 2+ | NN or GNN | GNN Lite (4,033 params) | FPGA実装、hardware-adaptive |

---

## 8. 次世代デコーダ技術（最新研究）

### 8.1 FPGA NNデコーダ（Phase 0+候補）
...
### 8.2 qLDPC-GKP移行（Phase 2+ロードマップ）
