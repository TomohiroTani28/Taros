...
| 消費電力 (WDM込み) | **~104W** | **~109W** | **~112W** | Edu OPA×2必須(macronode要件)で100→104W
...
**Tゲートレート モード区分**: 上表の値はstrict postselection(全QECラウンド高信頼)モード。
全モード(postselectionなし)では最小間隔=7.42μs(d=7)→単一ch ~135kHz, 7ch ~945kHz。
...
**約630万円 Phase -1実験（G-EXP1）がLevel A確率を50%超に引き上げる最重要マイルストーン。**
...
```
Phase -1 (開始後0-6ヶ月): CV固有: 約630万円 GKP実験 + 約4,200万円基盤 = 約4,830万円
    │
...
Strict postselection: 10⁶ / (146Hz × 7ch) ≈ **16分**。
...
d=7で10⁶ Tゲートの1,000 qubit回路の実行時間は全モード~1秒 / strict~16分（WDM 7ch時）。
