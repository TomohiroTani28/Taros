...
**Document ID**: PQC-CV-EXEC-v2.0 [v3.6]
...
| 実効スクイージング | σ_eff | **5.0dB** (現離散光学) / 8.5dB (Phase 1, 13dB) / **≈9.3dB** (Phase 2+ PIC 現実的, L=0.27dB, non-loss込み) / 10.8dB (理論限界, L=0.15dB) | v3.6修正: Phase 1を8.5dBに、Phase 2+ PIC現実的を9.3dBに修正。理論限界10.8dB。 | 06_noise-budget v3.6 |
| 物理エラー率 | p_phys | 現離散(5.0dB): 閾値未達 / Phase 1(13dB, 8.5dB): 7.5×10⁻³ / Phase 2+ PIC現実的(13dB, L=0.27dB): ~4.9×10⁻³ | v3.6修正: p_phys 4.9×10⁻³ for L=0.27dB | 13_performance |
...
| 論理エラー率 d=7 (Δ=0理想) | p_L | **6.1×10⁻⁷ (MWPM, Phase 2+ PIC)** [理論上限値] | v3.6: 製品目標p_L≲10⁻³(L=0.27dB)。現実的PIC(L=0.27dB, QE≥99%)でMWPM~3.3×10⁻⁴。L≤0.22dBで≲10⁻⁴。UF~5×10⁻⁴(超過)→MWPM必須 | 13_performance |
| 論理エラー率 d=7 (現実Δ=0.12) | p_L | **~3.3×10⁻⁴ (MWPM, L=0.27dB)** [対外基準値] | v3.6修正: 13_performance §3シナリオ「現実」行 | 13_performance |
...
| 閾値マージン | — | v3.6修正: 現離散光学→閾値未達; Phase 1→+1.0dB(PS)/−1.5dB(全モード); Phase 2+ PIC→+3.3dB(PS)/+0.8dB(全モード) | vs 7.5dB / 10.0dB [v3.6: Phase 1は8.5dB基準] | 06_noise-budget v3.6 |
...
| 電源 | — | USB-PD EPR 140W (28V/5A) 推奨 | Pro 109W / Max 112W で100W不可。Edu(104W)は100W超 [v3.5: OPA×2で100→104W] | 10_portable |
...
|---|:---:|:---:|:---:|
| 表面符号距離 | d=3 | d=5 | d=7 |
| 論理エラー率 (MWPM, Phase 2+ PIC現実的) | — | — | **~3.3×10⁻⁴** (v3.6修正, σ_eff=9.3dB, L=0.27dB, MWPM d=7) |
...
| **実効スクイージング σ_eff** | **5.0dB (現離散光学) / 8.5dB (Phase 1, 13dB) / 10.8dB (Phase 2+ PIC理論限界)** | v3.6修正: Phase 1を8.5dB、理論限界を10.8dBに統一。現実的(L=0.27dB)は9.3dB |
...
| **閾値マージン (Phase 1, postsel)** | **+1.0dB** | 8.5 - 7.5 (break-even境界) [v3.6: 8.8→8.5dB修正] |
| **閾値マージン (現離散光学, Option A+AWG)** | **閾値未達** | 5.0dB < 7.5dB。**注: Phase -1実験はOption B(AWGなし, L=0.39dB, σ_eff=8.5dB)で実施。閾値超過+1.0dB** |
| 物理エラー率 p_err (v3.6) | Phase 1(13dB): 7.5×10⁻³ / Phase 2+ PIC現実的(13dB, L=0.27dB): ~4.9×10⁻³ / 理論限界(L=0.15dB): 9.9×10⁻⁴ | v3.6修正 |
| 表面符号閾値 p_th_eff | 1.5% | soft-info MWPM (Noh&Chamberland 2022); UF時≈0.8-1.2%. v2.1: 製品デコーダはMWPM |
| **論理エラー率 p_L (d=7, Phase 2+ PIC)** | **製品仕様: ~3.3×10⁻⁴ (L=0.27dB, QE≥99%, MWPM)** / L≤0.22dBで≲10⁻⁴ / 理論限界: 6.1×10⁻⁷ | v3.6: non-loss noise込み統一計算。L=0.27dBでp_L=3.3×10⁻⁴(≲10⁻³)。L≤0.22dBで≲10⁻⁴達成。理論限界(L=0.15dB, Δ=0) 6.1×10⁻⁷。UFは現実的条件で~5×10⁻⁴(超過)→MWPM必須 |
...
**$42K Phase -1実験（G-EXP1）がLevel A確率を50%超に引き上げる最重要マイルストーン。**
...
4. **閾値マージン (v3.6修正)**: 現離散光学(σ_eff=5.0dB)では閾値未達。Phase 1(8.5dB, 13dB)でbreak-even境界、Phase 2+ PIC現実的(≈9.3dB, L=0.27dB)で製品グレード境界、理論限界(10.8dB, L=0.15dB)で十分マージン。ロードマップ実行が必須
...
```
2026 Q2-Q4: Phase -1 (CV固有: $42K GKP実験 + $280K基盤 = $322K)
    │
    ├── G-EXP1 (7月): CW GKP F>0.80, p_acc>85%, Δ<0.15 [Option B, AWGなし, σ_eff=8.5dB]
