...
│    → 実際のTaros Edu プロトタイプ                         │ $370K (Level 1-4累計)
├─────────────────────────────────────────────────────────┤
│ Level 3: TDM macronode クラスタ生成                       │ 光学実験
│    → OPA×2 + BS×3 + delay + homodyne                    │ $80K
├─────────────────────────────────────────────────────────┤
│ Level 2: GKP状態生成 + soft-info取得                     │ 光学実験
│    → OPA×2 + homodyne + postselection                   │ $39K (G-EXP1)
├─────────────────────────────────────────────────────────┤
│ Level 1: Decoder / QEC シミュレーション                    │ 古典計算
│    → Stim + Python + GPU                                │ $5-20K
└─────────────────────────────────────────────────────────┘
```
...
| 項目 | 費用 | 備考 |
|------|------|------|
| GPU計算 (AWS p4d.24xlarge, 500h) | $16K | $32/h × 500h |
| 開発工数 (2名×2ヶ月) | $40K | decoder実装+検証 |
| ソフトウェアライセンス | $0 | Stim/PyMatching OSS |
| **Level 1 合計** | **$56K** | **期間: 2ヶ月** |
...
| 項目 | 費用 | 備考 |
|------|------|------|
| PPLN OPA ×2 (NTT共同研究) | $6K | 単価$3K |
| 775nm pump LD (200mW DFB) | $2K | 実験用(TA不要) |
| Balanced homodyne detector ×2 | $8K | Thorlabs PDB480C相当 |
| ADC (14bit 1GSPS) ×2 | $6K | Teledyne SP Devices |
| 光学部品 (BS, ミラー, PBS, λ/2) | $5K | |
| PMF, コネクタ, マウント | $3K | |
| Master laser (1550nm) | $3K | |
| SHG module | $3K | |
| 光学テーブル・マウント (既存流用想定) | $0-3K | |
| **Level 2 合計** | **$39K** | **期間: 3ヶ月** |

> ※費用定義の統一: 装置BOM $42K (OPA×2正確計上) / 装置直接費 $58K (予備・消耗品込み) / 完全コスト $198K (人件費2名×5ヶ月込み)。本文書のLevel 2 $39Kは旧OPA×1構成の最小BOMに相当。
...
- **Go**: F_GKP > 0.80 (初期確認基準; 製品仕様は F>0.85), P_s > 5×10⁻⁴, squeezing > 12dB
...
| 項目 | 費用 | 備考 |
|------|------|------|
| Level 2装置（流用） | ($39K) | G-EXP1から継続 |
| 追加BS ×3 (ファイバカプラ) | $3K | |
| PMF 37m (short delay + long delay) | $0.5K | |
| 775nm TA (500mW) | $8K | 2-OPA同時ポンプ |
| PZT fiber stretcher ×2 | $4K | 位相ロック用 |
| EO gate (LNOI EOM) | $4K | |
| PLL制御基板 (PID + DAC) | $5K | |
| 追加homodyne detector ×2 | $8K | 計4ch |
| FPGA評価ボード (Versal VE2302) | $5K | リアルタイムFF試験 |
| EO comb + RF driver (25GHz) | $7K | WDM LO生成 |
| 追加ADC ×2 | $6K | |
| 光学テーブル追加スペース | $3K | |
| **Level 3 追加分** | **$53.5K** | |
| **Level 2+3 累計** | **$92.5K** | **期間: +3ヶ月 (累計6ヶ月)** |
...
|------|------|------|
| Level 3装置（流用） | ($92.5K) | |
| FPGA本格実装 (VE2302 + カスタム基板) | $15K | |
| DAC (16bit 12GSPS) + EO driver | $10K | |
| AWG (8ch, 低損失) | $3K | |
| 追加PD ×8 (WDM用) | $4K | |
| 追加ADC ×8 | $20K | |
| 制御ソフトウェア開発 (3名×4ヶ月) | $120K | |
| 筐体・組立 | $10K | |
| 較正・テスト期間の人件費 | $40K | |
| **Level 4 追加分** | **$222K** | |
| **Level 1-4 全累計** | **$370K** | **期間: 12ヶ月** |
...
```
                                                      累計コスト
Level 1: Stim simulation (decoder)       $56K  ──┬──── $56K     [2ヶ月]
                                                  │
Level 2: GKP光学実験 (G-EXP1)            $39K  ──┘──── $95K     [並列実行: 2ヶ月目から開始]
                                                │
                                                │ ※Level 1 & 2 は依存関係なし → 並列実行推奨
                                                │   並列時クリティカルパス: 6ヶ月 (Level 2律速)
                                                │
Level 3: TDM macronode                    $54K  ──────── $149K    [+3ヶ月 = 9ヶ月]
                                                │
Level 4: Full prototype (Taros Edu)      $222K  ──────── $370K    [+4ヶ月 = 13ヶ月]
```

> **v1.8修正**: Level 1(Stim)とLevel 2(光学)に物理的依存関係はない。並列実行で2ヶ月短縮。
...
| ゲート | 時期 | 判定基準 | 次ステップ |
|--------|------|---------|-----------|
| G1 | 2ヶ月後 | p_th_eff ≥ 1.0% (Stim) | Level 2開始 |
| G2 | 5ヶ月後 | F_GKP > 0.80 (光学) | Level 3開始 |
| G3 | 8ヶ月後 | Cluster nullifier > 3dB | Level 4開始 |
| G4 | 12ヶ月後 | p_L < p_phys (break-even) | 製品開発 |
...
| 手法 | コスト | 検証範囲 | 限界 |
|------|--------|---------|------|
| **Stim + Python (Level 1のみ)** | $5-20K (GPU rental) | Decoder, threshold | GKP実物なし |
| **Xanadu Strawberry Fields** | $0 (OSS) | GKP simulation | ノイズモデル簡略 |
| **Amazon Braket (simulator)** | $1-5K | 小規模QEC回路 | CV不可 |
| **Azure Quantum (IonQ/Quantinuum)** | $10-50K | 高精度QEC (trapped ion) | 光方式ではない |

### 最小構成（最安）で何がわかるか

**$5K (GPU rental 150h)** で Level 1 の核心部分:
...
| 全部でいくら？ | **$370K / 12ヶ月** (段階的Go/No-Go付き) |
| 最安で核心だけ？ | **$5K** (Stim GPU simulation、2週間) |
| Phase -1実験は？ | **$95K / 5ヶ月** (Level 1+2、G-EXP1含む) |

---

*本文書はIBM Quantum実機では検証不可能なCV方式固有の4項目（GKP符号、soft-info decoder、
TDM構造、macronode cluster）の検証方法と費用を定義したものである。最小$5K(Stim GPU)から
最大$370K(全統合プロトタイプ)まで段階的に検証可能。*
