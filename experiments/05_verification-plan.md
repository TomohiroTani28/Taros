...
│    → OPA×2 + homodyne + postselection                   │ 約560万円 (G-EXP1)
├─────────────────────────────────────────────────────────┤
...
- 実効 F_GKP と Δ の実測
- 成功確率 p_acc (単一モード acceptance rate) の実測
...
**Level 2 合計: 約560万円** (Phase -1a 約480万円 + Phase -1b 約80万円)。期間: 3ヶ月。

> ※費用SSOT: `experiments/01_gkp-optical.md` (G-EXP1) の装置BOM約560万円を正とする。
> 個別項目・内訳は experiments/01_gkp-optical.md §4 BOM表を参照。
> 装置直接費約800万円 (予備・消耗品込み) / 完全コスト約2,900万円 (人件費2名×5ヶ月込み)。
...
- **Go**: F_GKP > 0.80 (初期確認基準; 製品仕様は F>0.85),
  p_acc > 85% (Go最低基準; 設計前提93%。85-93%間ではd=7→d=9拡張 or QD追加で補償),
  squeezing > 12dB
...
| 項目 | 費用 | 備考 |
|------|------|------|
| Level 2装置（流用） | (約560万円) | G-EXP1から継続 (SSOT: experiments/01) |
...
- LO基底切替EOM 動作確認 (消光比 >30dB, 設計要件。初期目標>25dBでGo判定可)
...
Level 1: Stim simulation (decoder)     約840万円  ──┬── 約840万円    [2ヶ月]
                                                    │
Level 2: GKP光学実験 (G-EXP1)          約560万円  ──┘── 約1,400万円  [並列実行: 2ヶ月目から開始]
                                                │
                                                │ ※Level 1 & 2 は依存関係なし → 並列実行推奨
                                                │   並列時クリティカルパス: 6ヶ月 (Level 2律速)
                                                │
Level 3: TDM macronode                  約803万円  ──── 約2,203万円  [+3ヶ月 = 9ヶ月]
                                                │
Level 4: Full prototype (Taros Edu)   約3,330万円  ──── 約5,533万円  [+4ヶ月 = 13ヶ月]
```
...
> **注**: 本文書のG1-G4はLevel別の検証ゲートであり、`analysis/phase-minus1-execution.md` のG1-G5（マイルストーン連動判定）とは番号体系が異なる。対応: 本G1≈M3(Stim), 本G2≈M5(GKP), 本G3≈M8(OPA), 本G4≈M10(break-even)。
...
| **追加** | PD QE実測と損失バジェット検証 | 市販PD(QE~95%)での実効σ_eff測定。BSモデル基準で検証(SSOT: Phase 2+ PIC σ_eff≈9.3dB@QE99%。QE=95%の場合はσ_eff低下)。 | Phase -1a Level 2 |
