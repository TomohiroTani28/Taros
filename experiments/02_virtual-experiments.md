...
> **v2.2注記**: 現設計はV_anti=20 SNU (13dB anti-squeezed, 単一パスOPA)。
> 上表13dB行が設計値。
...
- **Tarosのσ_eff=11.5dB(旧dB減算値)はこの閾値を+4.0dB上回ると主張していたが、
  v2.0 BSモデルではσ_eff=10.9dB (Phase 2+ PIC)で閾値+3.4dBマージン。
...
| 閾値マージン | Walshe+ 2025 | v2.0 BSモデル: +3.4dB (Phase 2+ PIC 10.9dB vs 7.5dB postsel閾値)。旧値+4.0dB (11.5dB vs 7.5dB) はdB減算誤りに基づく | 3/5 |
