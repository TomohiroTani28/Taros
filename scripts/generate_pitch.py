#!/usr/bin/env python3
"""Generate TAROS Investor Pitch Deck v4.0 (17 slides)"""
...
add_text(s, 9.5, 6.8, 3.5, 0.4, "C o n f i d e n t i a l   |   v 4 . 0",
         size=10, color=TEXT_LIGHT, align=PP_ALIGN.RIGHT)

# ════════════════════════════════════════════════════
# SLIDE 2: VISION — 7-in-1
# ════════════════════════════════════════════════════
s = prs.slides.add_slide(prs.slide_layouts[6])
add_bg(s, BG_WHITE)
...
modes = [
    ...
    ("Mode 2", "CIM", "組合せ最適化 ~2,000スピン", "購入即日", GREEN),
    ("Mode 3", "QRC", "時系列予測・異常検知", "購入即日", GREEN),
    ("Mode 4", "QRNG", "量子乱数 ~200Mbps", "購入即日", GREEN),
    ("Mode 5", "Sensing", "SQL以下精密測定", "購入即日", GREEN),
    ("Mode 6", "Imaging", "サブショットノイズ顕微鏡", "購入即日", GREEN),
...
for i, (mode, name, desc, avail, clr) in enumerate(modes):
    ...
    # availability badge
    badge_color = clr
    add_rect(s, x + 0.08, y + 2.1, 1.3, 0.35, badge_color)
...
add_text(s, 0.6, 1.1, 12, 0.9,
         "量子コンピュータは「データセンター専用」で止まっている",
         size=34, bold=True, color=TEXT_DARK)
add_text(s, 0.6, 1.9, 12, 0.5,
         "超伝導・イオン方式は希釈冷凍機 (15mK) または高真空イオントラップを必須とし、原理的に小型化が不可能。",
         size=13, color=TEXT_GRAY)

labels = ["冷 却", "消 費 電 力", "サ イ ズ", "価 格"]
values = ["15mK ~ 4K", "30 ~ 200kW", "部屋規模", "4.5~22.5億円"]
descs = ["希釈冷凍機 / He冷凍機が必須", "施設の専用配電が必要",
         "既存建屋への設置に改装必要", "数台単位、購入は政府・大企業のみ"]

for i in range(4):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.8, 2.85, 2.2, CARD_BG)
    add_rect(s, x, 2.8, 2.85, Inches(0.04).inches, RED_SOFT)
    add_text(s, x + 0.15, 2.95, 2.55, 0.3, labels[i], size=11, color=TEXT_GRAY)
    add_text(s, x + 0.15, 3.3, 2.55, 0.5, values[i], size=26, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.15, 4.0, 2.55, 0.6, descs[i], size=10, color=TEXT_GRAY)

add_text(s, 0.6, 5.4, 12, 0.5,
...
for i in range(4):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.0, 2.85, 1.8, CARD_BG)
    add_text(s, x + 0.15, 2.15, 2.55, 0.3, sol_labels[i], size=11, color=TEXT_GRAY)
    add_text(s, x + 0.15, 2.45, 2.55, 0.5, sol_values[i],
             size=30, bold=True, color=ACCENT)
    add_text(s, x + 0.15, 3.05, 2.55, 0.4, sol_descs[i], size=10, color=TEXT_GRAY)

# Tech stack
add_text(s, 0.6, 4.2, 5.5, 0.35, "H O W  —  技 術 ス タ ッ ク",
...
for j, (t, d) in enumerate(tech):
    tf = add_text(s, 0.8, 4.55 + j * 0.5, 5.2, 0.5, f"  {t}", size=12, bold=True)
    add_para(tf, f"    {d}", size=10, color=TEXT_GRAY)

# Why Now
add_text(s, 6.8, 4.2, 6, 0.35, "W H Y   N O W  —  今 、 成 立 す る 理 由",
         size=11, bold=True, color=ACCENT)
why = [
...
for j, (t, d) in enumerate(why):
    tf = add_text(s, 7.0, 4.55 + j * 0.5, 5.8, 0.5, f"  {t}", size=12, bold=True)
    add_para(tf, f"    {d}", size=10, color=TEXT_GRAY)
...
for i, (title, subtitle, desc) in enumerate(day1_modes):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.4, 2.85, 3.2, CARD_BG)
    add_accent_line(s, x, 2.4, 2.85)
    add_text(s, x + 0.15, 2.55, 2.55, 0.3, title, size=14, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.15, 2.9, 2.55, 0.3, subtitle, size=11, color=ACCENT)
    add_text(s, x + 0.15, 3.3, 2.55, 1.8, desc, size=10, color=TEXT_GRAY)

add_rect(s, 0.6, 5.9, 12, 0.7, BG_DARK)
add_text(s, 0.8, 5.95, 11.6, 0.6,
         "投資家にとっての意味: FTQC完成前からMode 2-7で顧客価値を提供し、初期売上を確保。",
         size=14, bold=True, color=ACCENT, align=PP_ALIGN.CENTER)

# ════════════════════════════════════════════════════
# SLIDE 6: TECHNICAL EDGE
# ════════════════════════════════════════════════════
s = prs.slides.add_slide(prs.slide_layouts[6])
add_bg(s, BG_WHITE)
...
for i, (val, desc) in enumerate(chain):
    ...
    add_rect(s, x, 2.2, 2.3, 1.8, bg_c)
    add_text(s, x + 0.1, 2.3, 2.1, 0.5, val, size=22, bold=True, color=txt_c,
             align=PP_ALIGN.CENTER)
    add_text(s, x + 0.1, 2.85, 2.1, 0.8, desc, size=9,
             color=TEXT_WHITE if i == 4 else TEXT_GRAY, align=PP_ALIGN.CENTER)
    if i < 4:
        add_text(s, x + 2.15, 2.6, 0.4, 0.4, "→", size=18, bold=True, color=ACCENT)

# Threshold margin bar
add_text(s, 0.6, 4.4, 12, 0.4,
         "閾値マージン — Phase 2+ PIC 統合での量子誤り訂正成立余裕",
         size=14, bold=True, color=TEXT_DARK)
# Green bar (sigma_eff)
add_rect(s, 0.6, 5.0, 9.5, 0.45, GREEN)
add_text(s, 0.8, 5.02, 3, 0.4, "実効 σ_eff (Phase 2+ PIC現実)", size=10,
         color=TEXT_WHITE)
add_text(s, 9.2, 5.02, 1.5, 0.4, "9.3 dB", size=12, bold=True,
         color=TEXT_WHITE, align=PP_ALIGN.RIGHT)
# Purple bar (threshold)
add_rect(s, 0.6, 5.55, 7.2, 0.45, RGBColor(0x7E, 0x57, 0xC2))
add_text(s, 0.8, 5.57, 3, 0.4, "閾値 (postselection)", size=10, color=TEXT_WHITE)
add_text(s, 6.8, 5.57, 1.5, 0.4, "7.5 dB", size=12, bold=True,
         color=TEXT_WHITE, align=PP_ALIGN.RIGHT)
...
advs = [
    ("14bit soft-info",
     "全競合方式（超伝導/イオン/中性原子）は binary シンドローム。\n"
     "CV方式は14bitアナログ情報をデコーダに供給。\n"
     "MWPM閾値 1.5% は soft-info によって達成。\n"
     "同一物理エラー率で論理エラー率を桁違いに低減。"),
    ("qumode = 無限次元ヒルベルト空間",
     "ボソニック場理論をネイティブにシミュレーション。\n"
     "格子ゲージ理論、Bose-Hubbard、分子振動 —\n"
     "DV方式では原理的に到達できない計算空間。\n"
     "CVBQP ⊇ BQP が理論的に証明済み (ITCS 2025)。"),
    ("7-in-1 プラットフォーム",
     "同一HWで7つの動作モード。\n"
     "量子優位証明前から6モードで商用価値を提供。\n"
     "競合のどの方式でも実現不可能な多用途性。\n"
     "投資リスクを本質的に低減。"),
]
...
for i, (title, desc) in enumerate(advs):
    x = 0.6 + i * 4.05
    add_rect(s, x, 2.2, 3.85, 4.0, CARD_BG)
    add_accent_line(s, x, 2.2, 3.85)
    add_text(s, x + 0.2, 2.45, 3.45, 0.5, title, size=20, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.2, 3.1, 3.45, 2.5, desc, size=12, color=TEXT_GRAY)
...
for i, (name, badge, price, specs, use, d_val) in enumerate(products):
    ...
    add_rect(s, x, 2.1, 3.85, 4.2, BG_WHITE, border)
    add_text(s, x + 0.2, 2.2, 2.5, 0.4, name, size=20, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.2, 2.55, 2.5, 0.3, d_val, size=11, color=ACCENT)
    if badge:
        add_rect(s, x + 2.6, 2.2, 1.05, 0.3, HIGHLIGHT)
        add_text(s, x + 2.6, 2.2, 1.05, 0.3, badge, size=9, bold=True,
                 color=TEXT_WHITE, align=PP_ALIGN.CENTER)
    add_text(s, x + 0.2, 3.0, 3.45, 0.5, price, size=28, bold=True, color=TEXT_DARK)
    ...
    add_text(s, x + 0.2, 5.5, 3.45, 0.4, use, size=12, bold=True, color=TEXT_DARK)
...
# ════════════════════════════════════════════════════
# SLIDE 9: MARKET & COMPETITION
# ════════════════════════════════════════════════════
s = prs.slides.add_slide(prs.slide_layouts[6])
add_bg(s, BG_WHITE)
add_header(s, "M A R K E T   &   C O M P E T I T I O N", "9")
add_text(s, 0.6, 1.1, 12, 0.7,
...
for j, h in enumerate(comp_headers):
    x = 0.6 + j * 2.0
    add_rect(s, x, 1.95, 1.95, 0.35, BG_DARK)
    add_text(s, x + 0.08, 1.97, 1.8, 0.3, h, size=9, bold=True, color=TEXT_WHITE)
...
# TAM/SAM/SOM
add_text(s, 0.6, 4.3, 5, 0.35, "市場規模 — TAM / SAM / SOM", size=14, bold=True)

tam_data = [
    ("T A M", "1,250機関 · 225億円", "量子研究・開発全組織", CARD_BG),
    ("S A M", "650機関 · 約117億円", "CV / QEC / 教育適合顧客", CARD_BG),
    ("S O M   Y 3", "100機関 · 18億円", "3年目シェア 15% 獲得", BG_DARK),
]

for i, (label, val, desc, bg) in enumerate(tam_data):
    x = 0.6 + i * 4.05
    txt = TEXT_WHITE if i == 2 else TEXT_DARK
    add_rect(s, x, 4.8, 3.85, 1.5, bg)
    add_text(s, x + 0.2, 4.9, 3.45, 0.3, label, size=10, bold=True,
             color=ACCENT if i < 2 else ACCENT2)
    add_text(s, x + 0.2, 5.2, 3.45, 0.4, val, size=18, bold=True, color=txt)
    add_text(s, x + 0.2, 5.65, 3.45, 0.3, desc, size=10,
             color=TEXT_GRAY if i < 2 else TEXT_LIGHT)
...
for i, (big, sub, title, desc) in enumerate(validations):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.2, 2.85, 3.5, CARD_BG)
    add_text(s, x + 0.15, 2.35, 2.55, 0.6, big, size=36, bold=True, color=ACCENT)
    add_text(s, x + 0.15, 2.9, 2.55, 0.3, sub, size=10, color=TEXT_GRAY)
    add_text(s, x + 0.15, 3.3, 2.55, 0.4, title, size=13, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.15, 3.8, 2.55, 1.5, desc, size=10, color=TEXT_GRAY)
...
add_text(s, 0.6, 1.1, 12, 0.7,
         "段階的検証 → プロトタイプ → 量産", size=34, bold=True, color=TEXT_DARK)
...
for i, (name, dur, desc, cost, bg, txt) in enumerate(phases):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.0, 2.85, 2.5, bg)
    add_text(s, x + 0.2, 2.1, 2.45, 0.3, name, size=12, bold=True, color=ACCENT)
    add_text(s, x + 0.2, 2.4, 2.45, 0.25, dur, size=10, color=TEXT_GRAY if bg == CARD_BG else TEXT_LIGHT)
    add_text(s, x + 0.2, 2.8, 2.45, 0.5, desc, size=16, bold=True, color=txt)
    add_text(s, x + 0.2, 3.5, 2.45, 0.5, cost, size=24, bold=True,
             color=ACCENT if i == 0 else txt)

# Milestone
add_rect(s, 0.6, 5.0, 12, 0.8, CARD_BG)
add_text(s, 0.8, 5.05, 11.6, 0.7,
         "最重要マイルストーン: G-EXP1 (開始後2ヶ月) — 約800万円の装置直接費で Level A 成功確率を 25-35% から 50%超 へ引き上げ",
         size=13, bold=True, color=TEXT_DARK)
...
for i, (title, desc) in enumerate(risks):
    y = 2.0 + i * 0.95
    add_rect(s, 0.6, y, 7.0, 0.85, CARD_BG)
    add_accent_line(s, 0.6, y, 0.06)
    add_text(s, 0.8, y + 0.05, 6.6, 0.3, title, size=12, bold=True, color=TEXT_DARK)
    add_text(s, 0.8, y + 0.35, 6.6, 0.45, desc, size=10, color=TEXT_GRAY)

# Fallback & probability
add_rect(s, 8.2, 2.0, 4.4, 3.8, CARD_BG)
...
# Competitor funding
add_text(s, 6.8, 4.3, 5.5, 0.3, "競合の累計調達額 — 桁違いの資本効率",
         size=13, bold=True, color=TEXT_DARK)

comps = [
    ("PsiQuantum", "約1,500億円超"),
    ("IonQ", "約1,350億円"),
    ("Xanadu", "約560億円"),
    ("Taros (計画)", "約8.7億円"),
]

for i, (name, amount) in enumerate(comps):
    y = 4.7 + i * 0.45
    is_taros = i == 3
    add_text(s, 7.0, y, 2.5, 0.4, name, size=11,
             bold=is_taros, color=ACCENT if is_taros else TEXT_DARK)
    add_text(s, 10.0, y, 2.5, 0.4, amount, size=11,
...
add_text(s, 0.6, 1.1, 12, 0.7,
         "黒字化から36億円 ARR を視野", size=34, bold=True, color=TEXT_DARK)

fin = [
    ("黒 字 化", "Phase 2完了後1〜2年", "量産30–50台で達成"),
    ("粗 利 率", "~33%", "規模効果で逓増"),
    ("Y 3   A R R", "18億円", "100台 × 1,800万円"),
    ("Y 4   A R R", "36億円", "200台 (SAM 30%超)"),
]

for i, (label, val, desc) in enumerate(fin):
    x = 0.6 + i * 3.05
    add_rect(s, x, 2.0, 2.85, 1.8, CARD_BG)
    add_accent_line(s, x, 2.0, 2.85)
    add_text(s, x + 0.15, 2.15, 2.55, 0.25, label, size=10, color=TEXT_GRAY)
    add_text(s, x + 0.15, 2.45, 2.55, 0.5, val, size=22, bold=True, color=TEXT_DARK)
    add_text(s, x + 0.15, 3.0, 2.55, 0.4, desc, size=10, color=TEXT_GRAY)

add_rect(s, 0.6, 4.2, 12, 0.7, BG_DARK)
add_text(s, 0.8, 4.25, 11.6, 0.6,
         "投資論理: 失敗時上限 4.6億円 (Phase -1)、成功時 ARR 36億円 (Y4) — リスク調整後リターン ~3.5倍",
         size=14, bold=True, color=ACCENT, align=PP_ALIGN.CENTER)

# ════════════════════════════════════════════════════
# SLIDE 15: 2030 MOONSHOT
# ════════════════════════════════════════════════════
s = prs.slides.add_slide(prs.slide_layouts[6])
add_bg(s, BG_WHITE)
add_header(s, "2 0 3 0   M O O N S H O T", "15")
add_text(s, 0.6, 1.1, 12, 0.7,
         "Taros が拓く4つの未来産業", size=34, bold=True, color=TEXT_DARK)
...
add_text(s, 0.6, 1.1, 12, 0.7,
         "Phase -1 シード — 4.6億円 / 12ヶ月", size=34, bold=True, color=TEXT_DARK)
...
add_text(s, 0.8, 6.8, 8, 0.3,
         "T A R O S  ·  D e s k t o p   P h o t o n i c   Q u a n t u m   C o m p u t e r  ·  v 4 . 0",
         size=10, color=TEXT_LIGHT)
