...
> **「電源コンセントとLANケーブルで動く7-in-1量子プラットフォーム」**

超伝導方式（IBM/Google）はmK冷凍機が必須で原理的にデスクトップ化不可能。
フォトニック方式のみが到達可能な形態であり、Tarosの決定的差別化要因。

### 7-in-1 Quantum Platform

Tarosは**単一ハードウェアで7つの動作モードを提供**する世界初のプラットフォーム:
...
†初期SW設定1-2週間を含む。HW追加は不要。

Mode 2-7は購入初日から利用可能。Mode 1(FTQC)はファームウェア更新で段階的に進化。
詳細: `design/13_performance.md` §8

### CV方式の3つの決定的優位性

1. **14bit soft-info**: 全競合方式（超伝導/イオン/中性原子）のbinaryシンドロームに対し、デコーダ精度で本質的有利
2. **qumode = 無限次元ヒルベルト空間**: ボソニック場理論（格子ゲージ、Bose-Hubbard）をネイティブにシミュレーション
3. **7-in-1**: 同一ハードウェアで7つの動作モード — 量子優位証明前から6モードで商用価値を提供

### ムーンショットビジョン（Phase 2以降）

Tarosは4つの未来産業の**交差点**に位置する:
...
- **量子脳**: NeuroQフレームワーク（CV量子直接互換）でC.elegans 302ニューロンをリアルタイムシミュレーション
- **量子ホログラム**: QFTで干渉パターンを10⁶倍高速計算。GKPのフーリエ構造 = ホログラムのフーリエ構造
- **量子創薬**: プロトントンネリング（DNA変異の量子起源）解明、セノリティクス分子の量子設計
- **ロボット知能**: CIMで50ms制御壁突破、スクイーズド光でSQL超え力覚センシング

詳細: `design/13_performance.md` §9
...
| 論理エラー率 (d=7, Phase 2+ PIC) | **製品仕様: p_L ≈3.3×10⁻⁴ (L=0.27dB, QE≥99%, MWPM, V_non-loss込み)**†‡ / L≤0.22dBで≲10⁻⁴ / L≤0.17dBで<10⁻⁵ / 理論限界: 6.1×10⁻⁷ (全条件達成時) |
...
| 論理qubit | 10〜1,000+（TDM逐次処理†† ） |

†**3段階スペック定義**:
- Phase 1 (離散光学系, 13dB, L=0.39dB): p_L ~ 10⁻³ — QEC break-even実証
- Phase 2+ PIC (現実的, 13dB, L=0.27dB, QE≥99%): p_L ≈ 3.3×10⁻⁴ — 製品目標 (MWPM d=7)
- Phase 2+ PIC (L≤0.17dB, Δ<0.12, QE≥99%): p_L < 10⁻⁵ — 条件達成時
- 理論限界 (L=0.15dB, QE=99%, Δ=0): p_L ≈ 6.1×10⁻⁷ — 設計上限 (全条件同時達成時)

††TDM逐次処理。同時独立qubitはWDMチャネル数(5-8)が上限。

‡3.3×10⁻⁴はV_non-loss=0.010SNU（電子ノイズ+ADC+PMF+WDM+GAWBS）込み。有限エネルギーGKP(Δ)・ポンプRIN未包含。Δ=0.10, RIN=−150dB/Hz全込み推定: 5-9×10⁻⁴（製品要件p_L≲10⁻³は達成）。精密値はPhase -1実測で確定。

現実的推定は「5つの前提条件(Δ<0.12, OPA≥13dB, PLL≥500kHz, QE≥99%, L_total≤0.27dB)」の達成度に依存。
詳細: `design/13_performance.md` §3.2 性能予測表。
...
(design/06_noise-budget.md)                                       (design/13_performance.md)
```
