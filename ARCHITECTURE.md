...
| 論理qubitスケーリング | **TDM: 追加HW $0** (逐次処理: 1000 qubit回路の実行時間はd=7で~17分。並列処理はWDM拡張で対応) | SNSPD比例増加 | 物理qubit比例増加 |
...
4. **QDなし構成の成立**: pure CV（QD=0）でも Phase 2+ PIC時 p_L≈10⁻⁵ (MWPM, d=7, L=0.27dB現実的楽観) → 製品スペック。理論限界p_L≈5.7×10⁻⁷はL=0.15dB+QE99%+Δ=0の全条件達成時のみ（v3.1修正）

**ゲートレート注記**: 単一chでの実効CNOTレートは~21Hz(d=7, QECサイクル7回×146Hz/7)。実用的な量子化学計算(10⁶ Tゲート)は~17分で完了。WDM 7chで~1kHz Tゲート/~146Hz CNOT。
...
| パッシブ冷却 → 低速ファン | 2026-05 | 自然対流~34W < 必要106W | `design/12_mechanical.md` |
...
| ADC: 外部Flash → FPGA内蔵6bit | 2026-05 | レイテンシ9ns→2ns | `design/07_feedforward.md` |
