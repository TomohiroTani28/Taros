...
| 閾値マージン | v3.6修正: 現離散光学→閾値未達(5.0dB<7.5dB); Phase 2+ PIC→**+1.8dB**(PS)/−0.7dB(全モード) | -10pt | +30pt |
...
1. **閾値超過 (Phase 2+ PIC)**: v3.6修正: σ_eff≈9.3dB(Phase 2+ PIC, L=0.27dB) > σ_th=7.5dB(PS). マージン+1.8dB(PS)。全モード閾値10dBに対しては−0.7dB(理論限界L=0.15dBで10.8dB→+0.8dB)。現離散光学(σ_eff=5.0dB)では閾値未達→ロードマップ実行が必須
2. **室温動作**: 1550nm光子の熱雑音 n_th=3×10⁻¹⁴ → 完全に無視可能
3. **TDMスケーリング**: 論理qubit数が物理リソースに依存しない唯一の方式
4. **QDなし構成の成立**: pure CV（QD=0）でも Phase 2+ PIC時 p_L≈3.3×10⁻⁴ (MWPM, d=7, L=0.27dB, QE≥99%, non-loss noise込み) → ≲10⁻³。L≤0.17dBで<10⁻⁵確実達成。理論限界p_L≈6.1×10⁻⁷はL=0.15dB+QE99%+Δ=0の全条件達成時のみ（v3.6修正）
...
| Union-Find → MWPM復帰 (製品デコーダ) | 2026-05 | v3.6: 現実的PIC(L=0.27dB)でMWPM d=7≈3.3×10⁻⁴。理論限界(L=0.15dB)ではMWPM 6.1×10⁻⁷で十分マージン。UFは実験用 | `design/08_decoder.md` |
...
| **v3.6統合修正 (Session 14+)** | 2026-05-06 | **σ_eff 9.5→9.3dB**(L=0.27dB, non-loss込み再計算)。**Phase 1 σ_eff 8.8→8.5dB**。**理論限界10.9→10.8dB**。**p_L修正**: 7×10⁻⁵→3.3×10⁻⁴(L=0.27dB)、5.7×10⁻⁷→6.1×10⁻⁷(理論限界)。**p_err 3×10⁻³→4.9×10⁻³**。**L≤0.20dB→L≤0.17dBで<10⁻⁵**。閾値マージン再計算。 | 全design/, README, ARCHITECTURE |
