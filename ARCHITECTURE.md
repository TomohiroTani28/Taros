...
| 閾値マージン | 現離散光学→閾値未達(5.0dB<7.5dB); Phase 2+ PIC→**+3.3dB**(PS)/+0.8dB(全モード, 理論限界時) | -10pt | +30pt |
...
1. **閾値超過 (Phase 2+ PIC)**: σ_eff≈9.3dB(Phase 2+ PIC, L=0.27dB) > σ_th=7.5dB(PS). マージン+3.3dB(PS)。全モード閾値10dBに対しては理論限界L=0.15dBで10.8dB→+0.8dB。現離散光学(σ_eff=5.0dB)では閾値未達→ロードマップ実行が必須
