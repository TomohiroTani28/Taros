...
| 論理qubitスケーリング | **TDM: 追加HW 0円** (逐次処理: 1000 qubit回路(10⁶ Tゲート)の実行時間はd=7で全モード~1秒 / strict~20分。並列処理はWDM拡張で対応) | SNSPD比例増加 | 物理qubit比例増加 |
...
**ゲートレート注記**: 単一chでの実効CNOTレートは~21Hz(d=7, QECサイクル7回×146Hz/7)。実用的な量子化学計算(10⁶ Tゲート)は全モード~1秒 / strict~20分で完了(WDM 7ch時)。WDM 7chで~840Hz Tゲート(strict)/~135kHz(全モード)/~146Hz CNOT。
